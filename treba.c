...
static pthread_mutex_t g_loglikelihood_mutex = PTHREAD_MUTEX_INITIALIZER;
...
void hmm_normalize(struct hmm *hmm) {
    ...
    /* Normalize by source state so each state's outgoing transition mass sums to 1. */
    for (source = 0; source < hmm->num_states - 1; source++) {
	for (target = 1, sum = 0.0; target < hmm->num_states; target++) {
...
PROB hmm_sum_transition_prob(struct hmm *hmm, int state) {
    ...
    sum = 0;
    for (i = 0; i < hmm->num_states; i++) {
	if (*HMM_TRANSITION_PROB(hmm, state, i) >= SMRZERO_LOG)
...
PROB hmm_sum_emission_prob(struct hmm *hmm, int state) {
	...
	if (*HMM_EMISSION_PROB(hmm, state, i) >= SMRZERO_LOG)
...
void *trellis_fill_bw(void *threadargs) {
	...
	backward_prob = trellis_backward(trellis, obs->data, obs->size, fsm);
	forward_prob = trellis_forward_fsm(trellis, obs->data, obs->size, fsm);
	pthread_mutex_lock(&g_loglikelihood_mutex);
	g_loglikelihood += backward_prob * occurrences;
	pthread_mutex_unlock(&g_loglikelihood_mutex);
...
PROB train_baum_welch_hmm(struct hmm *hmm, struct observations *o, int maxiterations, PROB maxdelta, int vb) {
    struct trellis *trellis, *trellisarray[MAX_BW_THREADS];
    struct thread_args *threadargs[MAX_BW_THREADS];
    ...
    pthread_t threadids[MAX_BW_THREADS];
    
    if (g_num_threads > MAX_BW_THREADS) {
	fprintf(stderr, "WARNING: Clamping threads to %i (requested %i)\n", MAX_BW_THREADS, g_num_threads);
	g_num_threads = MAX_BW_THREADS;
    } else if (g_num_threads < 1) {
	g_num_threads = 1;
    }
...
static struct option long_options[] =
 ...
 while ((opt = getopt_long(argc, argv, "a:b:d:f:g:hl:i:o:p:r:t:uvx:y:A:CD:G:HL:M:RT:", long_options, &option_index)) != -1) {
	...
	case 't':
	    ...
	    if (g_num_threads > MAX_BW_THREADS) { g_num_threads = MAX_BW_THREADS; }
	    break;
	case 'y':
	    g_t0 = atoi(optarg);
	    break;
